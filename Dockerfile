...
RUN chmod g+rwx /var/run/nginx.pid && \
	...
	chmod +rx /sbin/entrypoint.sh && \
	chmod +r /etc/supervisor/supervisord.conf /etc/nginx/nginx.conf /etc/php7/php-fpm.d/www.conf /etc/nginx/conf.d/default.conf /var/www/html/.env 
USER 1001
